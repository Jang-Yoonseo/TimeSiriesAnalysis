...
mean(abs(kings[-1] - kingstimeseriesSMA3[-42]) / kings[-1], na.rm = T) *
  100 ##MAPE
#여기 부분 다시 공부해보기
...
mean(abs(kings[-1] - SMA(kingstimeseries, n = 10)[-42]) / kings[-1], na.rm = T) *
  100 ##MAPE
...
#평활 상수에 따른 SSE 비교
SSE_ses <- sapply(w, function(alpha)
  HoltWinters(
    mindex,
    alpha = alpha,
    beta = FALSE,
...
legend(
  "topright",
  legend = c("Mindex", expression(alpha == 0.9)),
  lty = 1:2,
  col = c("black", "red")
)
...
plot(
  resid(fit5),
  main = expression("residual plot: " ~ alpha ~ ","
                    ,  ~ beta ~ ": estimated"),
  cex.main = 5
)
